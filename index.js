...
app.post('/senderVier' , function ( req, res){
  console.log("Eingehende POST request");
  temp = req.body.temperatur;
  feucht = req.body.feuchtigkeit;
  console.log('Temperatur: ' + temp + ' Feuchtigkeit: ' + feucht);
  broadcastSenderVier(feucht, temp);
  res.sendStatus(200);
...
wss.on("connection", ws => {
  ...
  function broadcastSenderVier(feucht, temp) {
    wss.clients.forEach(function each(client) {
      if (client.readyState === WebSocket.OPEN) {
        client.send(JSON.stringify({ type: 'feuchtigkeitS4', value: feucht }));
        client.send(JSON.stringify({ type: 'temperaturS4', value: temp }));
      }
    });
  }